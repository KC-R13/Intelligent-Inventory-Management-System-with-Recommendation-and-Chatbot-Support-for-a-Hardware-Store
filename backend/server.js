...
app.post(['/api/chatbot/chat', '/assistant/chat'], async (req, res) => {
  try {
    ...
    // Enhanced pattern matching with better natural language support (normalized tokens)
    if (normalized.includes('show plumbing products') || normalized.includes('plumbing') || normalized.match(/\b(pipe|tubing|faucet|drain)\b/)) {
    ...
    } else if (normalized.includes('power tools') || normalized.includes('power tool') || normalized.match(/\b(drill|saw|sander|grinder)\b/)) {
      intent = 'category_search'
      const [rows] = await db.query(
        `SELECT p.sku, p.name, p.sell_price FROM products p
         LEFT JOIN categories c ON c.id = p.category_id
    ...
    } else if (normalized.includes('most sold') || normalized.includes('popular') || normalized.includes('best selling')) {
    ...
    } else if (normalized.includes('tell me more about') || normalized.includes('details about') || normalized.includes('info about')) {
    ...
    } else if (normalized.includes('stock') || normalized.includes('inventory') || normalized.match(/\b(how many|quantity|available)\b/)) {
      intent = 'stock_check'
      if (normalized.includes('how many') && (normalized.includes('low') || normalized.includes('out of stock'))) {
        const [rows] = await db.query(
    ...
    } else if (normalized.includes('price') || normalized.includes('cost') || normalized.includes('expensive') || normalized.includes('cheap') || normalized.includes('cheapest')) {
      intent = 'pricing'
      if (normalized.includes('expensive') || normalized.includes('highest') || normalized.includes('most expensive')) {
        const [rows] = await db.query(
    ...
    } else if (normalized.includes('sales') || normalized.includes('revenue') || normalized.includes('sold') || normalized.includes('sales by category')) {
      intent = 'sales_analytics'
      if (normalized.includes('category') || normalized.includes('by category') || normalized.includes('sales by category')) {
        const [rows] = await db.query(
    ...
    } else if (normalized.includes('category') || normalized.includes('categories') || normalized.includes('what categories') || normalized.includes('show categories')) {
      intent = 'categories'
      const [rows] = await db.query(
    ...
    } else if (normalized.includes('help') || normalized.includes('what can you') || normalized.includes('commands')) {
      intent = 'help'
